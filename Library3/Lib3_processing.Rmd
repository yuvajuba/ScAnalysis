...
## Directories :
if(params$c3m == T){
  out_object_dir <- "~/Bureau/Juba/Projects/SingleCell/Library3/Out_objects/"
  out_figure_dir <- "~/Bureau/Juba/Projects/SingleCell/Library3/Out_figures/"
  out_seurat_dir <- "~/Bureau/Juba/Projects/SingleCell/Library3/SeuratObjects/"
} else {
...
```{r}
if(params$c3m == T){
  SeuratObj <- readRDS("~/Bureau/Juba/Projects/SingleCell/Library3/SeuratObjects/Seurat2.rds")
} else {
...
cells_rm <- WhichCells(SeuratObj, 
                       expression = umap_1 > 5 | umap_2 > 6 | 
                         Clust_res0.2 %in% c("1D","2R") | clone_id %in% c("unique","other"))
...
SeuratObj_filt@meta.data <- SeuratObj_filt@meta.data %>% 
  mutate(MyClusters = case_when(Clust_res0.2 %in% c("2D") ~ "Diag1",
                                Clust_res0.2 %in% c("0R","1R") & umap_1 > 0.2 & umap_2 > -2.5 & umap_2 < 0 ~ "Rel_in_Diag1",
                                Clust_res0.2 %in% c("1R","0R") ~ "Rel1",
                                Clust_res0.2 %in% c("0D") & umap_1 <= 0 ~ "Diag_in_Rel1",
                                Clust_res0.2 == "5R" ~ "Rel2",
                                Clust_res0.2 == "5D" ~ "Diag2",
                                TRUE ~ "others"))
