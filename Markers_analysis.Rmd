...
<p class='p3'>1-1- Top100 UpGenes from General conditions (all relapse __vs__ all diagnosis)</p>

```{r}
Markers1_lib1$lib1_R_vs_D$Up %>% head(100)
```
...
xx_kegg <- compareCluster(list(`R-D` = tmp_ENTREZ$`R-D_ENTREZ`$ENTREZID,
                               ...
                               `IC-D` = tmp_ENTREZ$`IC-D_ENTREZ`$ENTREZID), fun = "enrichKEGG", organism = "hsa", pvalueCutoff = 0.05) %>% 
  pairwise_termsim()
```
...
xx_kegg@compareClusterResult %>% 
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        axis.text = element_text(size = 13, colour = "navy", face = "bold"),
...
ggplot(dat, aes(x= Cluster, y= Description, fill= RichFactor))+
  ...
  theme(panel.grid = element_blank(),
        plot.margin = margin(r=2.5, l=0.6, t=0.3, b=0.2, unit = "in"),
        axis.text.x = element_text(size = 14, face = "bold", colour = "navy"),
...
```{r ,fig.height=13, fig.width=10, echo=TRUE}
treeplot(xx, 
         showCategory = 35, 
         cex_category = 4,
         hilight = F,
         fontsize = 4,
         offset.params = list(bar_tree = rel(3.3), tiplab = rel(2), hexpand = 0.05, extend = 0.45),
         cluster.params = list(n= 8,
                               method = "ward.D",
                               color = MyPalette,
                               label_words_n = 2,
  ...
  theme(legend.title = element_text(size = 14, face = "bold", colour = "darkred", margin = margin(b=0.2, unit = "in")),
        legend.text = element_text(size = 12, colour = "black"),
        plot.margin = margin(b=0.1, unit = "in"))
```
