...
for(i in names(Markers1_lib1)){
  dat <- Markers1_lib1[[i]][["all_DEA"]] %>% 
    ...
    dplyr::select(Genes,
                avg_log2FC,
                p_val_adj) %>% 
    dplyr::filter(abs(avg_log2FC) > 0.2) %>% 
...
## List gsea objects :
######################
GSEAobj_list <- list(Lib1 = list(), Lib2 = list(), Lib3 = list())
```
...
```{r ,rows.print=6}
if(nrow(gsea.go.bp@result) != 0){
  gsea.go.bp@result %>% dplyr::select(Description, setSize, NES, p.adjust, rank) %>% as.tibble()
} else {
  cat(c("The analysis showed no significant results", paste0("Length : ",nrow(gsea.go.bp@result))), sep = "\n")
}
```
...
```{r}
GSEAobj_list$Lib1[["R-allD"]] <- list(go = gsea.go.bp, 
                                      kegg = gsea.kegg)
```
...
```{r}
Markers1_lib1$lib1_allR_vs_D$Up %>% head(50)
```
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_R_vs_D_ICfree_Up, 
                     showCategory = 20, 
                     foldChange = Markers1_lib1$lib1_allR_vs_D$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8),
        axis.text = element_text(face = "bold"))+
  scale_fill_gradientn(colours = c("white","black"), values = c(0,1))
```
...
```{r}
Markers1_lib1$lib1_allR_vs_D$Down %>% head(50)
```

<p class='p3'>Enriched terms with the downgenes</p>

```{r ,rows.print=6}
GOobj1_lib1$GOobj_lib1_R_vs_D_ICfree_Down@result %>% 
  dplyr::filter(p.adjust < 0.05) %>% 
  dplyr::mutate(RichFactor = round(Count / as.numeric(sub("/\\d+","",BgRatio)),4)) %>% 
  dplyr::select(Description, RichFactor, Count, BgRatio) %>% 
  dplyr::arrange(desc(RichFactor)) %>% 
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_R_vs_D_ICfree_Down, 
                     showCategory = 15, 
                     foldChange = Markers1_lib1$lib1_allR_vs_D$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8, face = "bold"),
        axis.text.y = element_text(face = "bold", size = 10))+
...
```{r}
gsea.go.bp <- gseGO(geneList  = List_AllRankedGenes$lib1_allR_vs_D,
                 OrgDb        = org.Hs.eg.db,
                 ont          = "BP",
                 minGSSize    = 50,
                 maxGSSize    = 1000,
...
```{r ,fig.width=10}
gseaplot2(gsea.go.bp, geneSetID = c("GO:0002292", "GO:0046632", "GO:1902106", "GO:0032602"), 
          color = MyPalette[1:4], pvalue_table = F, subplots = 1:2, base_size = 13, rel_heights = c(2,0.5,1))
```
...
```{r ,fig.width=10}
gseaplot2(gsea.go.bp, geneSetID = c("GO:1903046", "GO:0000070", "GO:0051784", "GO:0051321"), 
          color = MyPalette[1:4], pvalue_table = F, subplots = 1:2, base_size = 14, rel_heights = c(2,0.5,1))
```
...
```{r}
gsea.kegg <- gseKEGG(geneList = List_AllRankedGenes$lib1_allR_vs_D,
                        maxGSSize = 1000) %>% 
  setReadable(OrgDb = 'org.Hs.eg.db', keyType = "ENTREZID")
```
...
```{r ,fig.width=10}
gseaplot2(gsea.kegg, geneSetID = c("hsa05330", "hsa05321", "hsa04659", "hsa04014"), 
          color = MyPalette[1:4], pvalue_table = F, subplots = 1:2, base_size = 13, rel_heights = c(2,0.5,1))
```
...
```{r}
GSEAobj_list$Lib1[["R-D"]] <- list(go = gsea.go.bp, 
                                   kegg = gsea.kegg)
```
...
```{r}
Markers1_lib1$lib1_IC_vs_D$Up %>% head(50)
```
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_IC_vs_D_Up, 
                     showCategory = 20, 
                     foldChange = Markers1_lib1$lib1_IC_vs_D$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8),
        axis.text = element_text(face = "bold"))+
  scale_fill_gradientn(colours = c("white","black"), values = c(0,1))
```
...
```{r}
Markers1_lib1$lib1_IC_vs_D$Down %>% head(50)
```

<p class='p3'>Enriched terms with the downgenes</p>

```{r ,rows.print=6}
GOobj1_lib1$GOobj_lib1_IC_vs_D_Down@result %>% 
  dplyr::filter(p.adjust < 0.05) %>% 
  dplyr::mutate(RichFactor = round(Count / as.numeric(sub("/\\d+","",BgRatio)),4)) %>% 
  dplyr::select(Description, RichFactor, Count, BgRatio) %>% 
  dplyr::arrange(desc(RichFactor)) %>% 
...
```{r}
gsea.go.bp <- gseGO(geneList  = List_AllRankedGenes$lib1_IC_vs_D,
                 OrgDb        = org.Hs.eg.db,
                 ont          = "BP",
                 minGSSize    = 50,
                 maxGSSize    = 1000,
...
```{r}
gsea.kegg <- gseKEGG(geneList = List_AllRankedGenes$lib1_IC_vs_D,
                        maxGSSize = 1000) %>% 
  setReadable(OrgDb = 'org.Hs.eg.db', keyType = "ENTREZID")
```
...
if(nrow(gsea.kegg@result) != 0){
  ...
  cat(c("The analysis showed no significant results", paste0("Length : ",nrow(gsea.kegg@result))), sep = "\n")
}
```
...
```{r}
GSEAobj_list$Lib1[["IC-D"]] <- list(go = gsea.go.bp, 
                                    kegg = gsea.kegg)
```
...
```{r}
Markers1_lib1$lib1_allR_vs_IC$Up %>% head(50)
```
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_R_vs_IC_Up, 
                     showCategory = 20, 
                     foldChange = Markers1_lib1$lib1_allR_vs_IC$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8),
        axis.text = element_text(face = "bold"))+
  scale_fill_gradientn(colours = c("white","black"), values = c(0,1))
```
...
```{r}
Markers1_lib1$lib1_allR_vs_IC$Down %>% head(50)
```

<p class='p3'>Enriched terms with the downgenes</p>

```{r ,rows.print=6}
GOobj1_lib1$GOobj_lib1_R_vs_IC_Down@result %>% 
  dplyr::filter(p.adjust < 0.05) %>% 
  dplyr::mutate(RichFactor = round(Count / as.numeric(sub("/\\d+","",BgRatio)),4)) %>% 
  dplyr::select(Description, RichFactor, Count, BgRatio) %>% 
  dplyr::arrange(desc(RichFactor)) %>% 
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_R_vs_IC_Down, 
                     showCategory = 15, 
                     foldChange = Markers1_lib1$lib1_allR_vs_IC$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8, face = "bold"),
        axis.text.y = element_text(face = "bold", size = 10))+
...
```{r}
gsea.go.bp <- gseGO(geneList  = List_AllRankedGenes$lib1_allR_vs_IC,
                 OrgDb        = org.Hs.eg.db,
                 ont          = "BP",
                 minGSSize    = 50,
                 maxGSSize    = 1000,
...
```{r}
gsea.kegg <- gseKEGG(geneList = List_AllRankedGenes$lib1_allR_vs_IC,
                        maxGSSize = 1000) %>% 
  setReadable(OrgDb = 'org.Hs.eg.db', keyType = "ENTREZID")
```
...
```{r}
GSEAobj_list$Lib1[["R-IC"]] <- list(go = gsea.go.bp, 
                                   kegg = gsea.kegg)
```
...
```{r}
Markers1_lib1$lib1_Rcl1_vs_Dcl1$Up %>% head(50)
```
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_Rcl1_vs_Dcl1_Up, 
                     showCategory = 20, 
                     foldChange = Markers1_lib1$lib1_Rcl1_vs_Dcl1$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8),
        axis.text = element_text(face = "bold"))+
  scale_fill_gradientn(colours = c("white","black"), values = c(0,1))
```
...
```{r}
Markers1_lib1$lib1_Rcl1_vs_Dcl1$Down %>% head(50)
```

<p class='p3'>Enriched terms with the downgenes</p>

```{r ,rows.print=6}
GOobj1_lib1$GOobj_lib1_Rcl1_vs_Dcl1_Down@result %>% 
  dplyr::filter(p.adjust < 0.05) %>% 
  dplyr::mutate(RichFactor = round(Count / as.numeric(sub("/\\d+","",BgRatio)),4)) %>% 
  dplyr::select(Description, RichFactor, Count, BgRatio) %>% 
  dplyr::arrange(desc(RichFactor)) %>% 
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_Rcl1_vs_Dcl1_Down, 
                     showCategory = 15, 
                     foldChange = Markers1_lib1$lib1_Rcl1_vs_Dcl1$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8, face = "bold"),
        axis.text.y = element_text(face = "bold", size = 10))+
...
```{r}
gsea.go.bp <- gseGO(geneList  = List_AllRankedGenes$lib1_Rcl1_vs_Dcl1,
                 OrgDb        = org.Hs.eg.db,
                 ont          = "BP",
                 minGSSize    = 50,
                 maxGSSize    = 1000,
...
```{r}
gsea.kegg <- gseKEGG(geneList = List_AllRankedGenes$lib1_Rcl1_vs_Dcl1,
                     maxGSSize = 1000,
                     nPermSimple = 20000) %>% 
  setReadable(OrgDb = 'org.Hs.eg.db', keyType = "ENTREZID")
```
...
```{r}
GSEAobj_list$Lib1[["Rcl1-Dcl1"]] <- list(go = gsea.go.bp, 
                                         kegg = gsea.kegg)
```
...
```{r}
Markers1_lib1$lib1_Rcl1_vs_ICcl1$Up %>% head(50)
```
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_Rcl1_vs_ICcl1_Up, 
                     showCategory = 20, 
                     foldChange = Markers1_lib1$lib1_Rcl1_vs_ICcl1$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8),
        axis.text = element_text(face = "bold"))+
  scale_fill_gradientn(colours = c("white","black"), values = c(0,1))
```
...
```{r}
Markers1_lib1$lib1_Rcl1_vs_ICcl1$Down %>% head(50)
```

<p class='p3'>Enriched terms with the downgenes</p>

```{r ,rows.print=6}
GOobj1_lib1$GOobj_lib1_Rcl1_vs_ICcl1_Down@result %>% 
  dplyr::filter(p.adjust < 0.05) %>% 
  dplyr::mutate(RichFactor = round(Count / as.numeric(sub("/\\d+","",BgRatio)),4)) %>% 
  dplyr::select(Description, RichFactor, Count, BgRatio) %>% 
  dplyr::arrange(desc(RichFactor)) %>% 
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_Rcl1_vs_ICcl1_Down, 
                     showCategory = 15, 
                     foldChange = Markers1_lib1$lib1_Rcl1_vs_ICcl1$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8, face = "bold"),
        axis.text.y = element_text(face = "bold", size = 10))+
...
```{r}
gsea.go.bp <- gseGO(geneList  = List_AllRankedGenes$lib1_Rcl1_vs_ICcl1,
                 OrgDb        = org.Hs.eg.db,
                 ont          = "BP",
                 minGSSize    = 50,
                 maxGSSize    = 1000,
...
```{r}
gsea.kegg <- gseKEGG(geneList = List_AllRankedGenes$lib1_Rcl1_vs_ICcl1,
                     maxGSSize = 1000,
                     nPermSimple = 20000) %>% 
  setReadable(OrgDb = 'org.Hs.eg.db', keyType = "ENTREZID")
```
...
```{r}
GSEAobj_list$Lib1[["Rcl1-ICcl1"]] <- list(go = gsea.go.bp, 
                                          kegg = gsea.kegg)
```
...
```{r}
Markers1_lib1$lib1_ICcl1_vs_Dcl1$Up %>% head(50)
```
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_ICcl1_vs_Dcl1_Up, 
                     showCategory = 20, 
                     foldChange = Markers1_lib1$lib1_ICcl1_vs_Dcl1$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8),
        axis.text = element_text(face = "bold"))+
  scale_fill_gradientn(colours = c("white","black"), values = c(0,1))
```
...
```{r}
Markers1_lib1$lib1_ICcl1_vs_Dcl1$Down %>% head(50)
```

<p class='p3'>Enriched terms with the downgenes</p>

```{r ,rows.print=6}
GOobj1_lib1$GOobj_lib1_ICcl1_vs_Dcl1_Down@result %>% 
  dplyr::filter(p.adjust < 0.05) %>% 
  dplyr::mutate(RichFactor = round(Count / as.numeric(sub("/\\d+","",BgRatio)),4)) %>% 
  dplyr::select(Description, RichFactor, Count, BgRatio) %>% 
  dplyr::arrange(desc(RichFactor)) %>% 
...
```{r ,fig.height=8, fig.width=10}
enrichplot::heatplot(GOobj1_lib1$GOobj_lib1_ICcl1_vs_Dcl1_Down, 
                     showCategory = 15, 
                     foldChange = Markers1_lib1$lib1_ICcl1_vs_Dcl1$All_FC)+
  theme(axis.text.x = element_text(angle = 90, size = 8, face = "bold"),
        axis.text.y = element_text(face = "bold", size = 10))+
...
```{r}
gsea.go.bp <- gseGO(geneList  = List_AllRankedGenes$lib1_ICcl1_vs_Dcl1,
                 OrgDb        = org.Hs.eg.db,
                 ont          = "BP",
                 minGSSize    = 50,
                 maxGSSize    = 1000,
...
```{r}
gsea.kegg <- gseKEGG(geneList = List_AllRankedGenes$lib1_ICcl1_vs_Dcl1,
                     maxGSSize = 1000,
                     nPermSimple = 20000) %>% 
  setReadable(OrgDb = 'org.Hs.eg.db', keyType = "ENTREZID")
```
...
```{r}
GSEAobj_list$Lib1[["ICcl1-Dcl1"]] <- list(go = gsea.go.bp, 
                                          kegg = gsea.kegg)
```
...
<p class='important'>First, let's inspect the genes between the different contrasts</p>

### 2-1- Inspecte common genes    {.tabset .tabset-pills}

```{r}
## Initiate an empty list to stock the results
Common_markers <- list()

# UP GENES :
############
## R-D vs Rcl1-Dcl1
tmp <- list(`R-D` = Markers1_lib1$lib1_allR_vs_D$Up,
            `Rcl1-Dcl1` = Markers1_lib1$lib1_Rcl1_vs_Dcl1$Up)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["R-D vs Rcl1-Dcl1 UP"]] <- p
## R-IC vs Rcl1-ICcl1
tmp <- list(`R-IC` = Markers1_lib1$lib1_allR_vs_IC$Up,
            `Rcl1-ICcl1` = Markers1_lib1$lib1_Rcl1_vs_ICcl1$Up)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["R-IC vs Rcl1-ICcl1 UP"]] <- p
## IC-D vs ICcl1-Dcl1
tmp <- list(`IC-D` = Markers1_lib1$lib1_IC_vs_D$Up,
            `ICcl1-Dcl1` = Markers1_lib1$lib1_ICcl1_vs_Dcl1$Up)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["IC-D vs ICcl1-Dcl1 UP"]] <- p

## R-D vs R-IC vs IC-D
tmp <- list(`R-D` = Markers1_lib1$lib1_allR_vs_D$Up,
            `R-IC` = Markers1_lib1$lib1_allR_vs_IC$Up,
            `IC-D` = Markers1_lib1$lib1_IC_vs_D$Up)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["R-D vs R-IC vs IC-D UP"]] <- p
## Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1
tmp <- list(`Rcl1-Dcl1` = Markers1_lib1$lib1_Rcl1_vs_Dcl1$Up,
            `Rcl1-ICcl1` = Markers1_lib1$lib1_Rcl1_vs_ICcl1$Up,
            `ICcl1-Dcl1` = Markers1_lib1$lib1_ICcl1_vs_Dcl1$Up)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 UP"]] <- p


# DOWN GENES :
##############
## R-D vs Rcl1-Dcl1
tmp <- list(`R-D` = Markers1_lib1$lib1_allR_vs_D$Down,
            `Rcl1-Dcl1` = Markers1_lib1$lib1_Rcl1_vs_Dcl1$Down)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["R-D vs Rcl1-Dcl1 Down"]] <- p
## R-IC vs Rcl1-ICcl1
tmp <- list(`R-IC` = Markers1_lib1$lib1_allR_vs_IC$Down,
            `Rcl1-ICcl1` = Markers1_lib1$lib1_Rcl1_vs_ICcl1$Down)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["R-IC vs Rcl1-ICcl1 Down"]] <- p
## IC-D vs ICcl1-Dcl1
tmp <- list(`IC-D` = Markers1_lib1$lib1_IC_vs_D$Down,
            `ICcl1-Dcl1` = Markers1_lib1$lib1_ICcl1_vs_Dcl1$Down)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["IC-D vs ICcl1-Dcl1 Down"]] <- p

## R-D vs R-IC vs IC-D
tmp <- list(`R-D` = Markers1_lib1$lib1_allR_vs_D$Down,
            `R-IC` = Markers1_lib1$lib1_allR_vs_IC$Down,
            `IC-D` = Markers1_lib1$lib1_IC_vs_D$Down)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["R-D vs R-IC vs IC-D Down"]] <- p
## Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1
tmp <- list(`Rcl1-Dcl1` = Markers1_lib1$lib1_Rcl1_vs_Dcl1$Down,
            `Rcl1-ICcl1` = Markers1_lib1$lib1_Rcl1_vs_ICcl1$Down,
            `ICcl1-Dcl1` = Markers1_lib1$lib1_ICcl1_vs_Dcl1$Down)
p <- Display_Venn(tmp, Padding = 0.06)
Common_markers[["Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 Down"]] <- p
```

#### R-D vs Rcl1-Dcl1

-   <p class='p2'>Up Genes</p>

```{r}
Common_markers$`R-D vs Rcl1-Dcl1 UP`$plot
```

The common genes are (top50): 

```{r}
Common_markers$`R-D vs Rcl1-Dcl1 UP`$intersections$`R-D ∩ Rcl1-Dcl1` %>% 
  head(50)
```

Specific to clonotype1 (top50)

```{r}
Common_markers$`R-D vs Rcl1-Dcl1 UP`$group_specific$`R-D | Rcl1-Dcl1`$`Rcl1-Dcl1` %>% 
  head(50)
```



-   <p class='p2', style='margin-top:100px'>Down Genes</p>

```{r}
Common_markers$`R-D vs Rcl1-Dcl1 Down`$plot
```

The common genes are (top50): 

```{r}
Common_markers$`R-D vs Rcl1-Dcl1 Down`$intersections$`R-D ∩ Rcl1-Dcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`R-D vs Rcl1-Dcl1 Down`$group_specific$`R-D | Rcl1-Dcl1`$`Rcl1-Dcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`R-IC vs Rcl1-ICcl1 UP`$plot
```

The common genes are (top50): 

```{r}
Common_markers$`R-IC vs Rcl1-ICcl1 UP`$intersections$`R-IC ∩ Rcl1-ICcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`R-IC vs Rcl1-ICcl1 UP`$group_specific$`R-IC | Rcl1-ICcl1`$`Rcl1-ICcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`R-IC vs Rcl1-ICcl1 Down`$plot
```

The common genes are (top50): 

```{r}
Common_markers$`R-IC vs Rcl1-ICcl1 Down`$intersections$`R-IC ∩ Rcl1-ICcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`R-IC vs Rcl1-ICcl1 Down`$group_specific$`R-IC | Rcl1-ICcl1`$`Rcl1-ICcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`IC-D vs ICcl1-Dcl1 UP`$plot
```

The common genes are (top50): 

```{r}
Common_markers$`IC-D vs ICcl1-Dcl1 UP`$intersections$`IC-D ∩ ICcl1-Dcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`IC-D vs ICcl1-Dcl1 UP`$group_specific$`IC-D | ICcl1-Dcl1`$`ICcl1-Dcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`IC-D vs ICcl1-Dcl1 Down`$plot
```

The common genes are (top50): 

```{r}
Common_markers$`IC-D vs ICcl1-Dcl1 Down`$intersections$`IC-D ∩ ICcl1-Dcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`IC-D vs ICcl1-Dcl1 Down`$group_specific$`IC-D | ICcl1-Dcl1`$`ICcl1-Dcl1` %>% 
  head(50)
```
...
```{r}
Common_markers$`R-D vs R-IC vs IC-D UP`$plot
```

__We have 286 up regulated genes from D to R. __

-   We have 29 genes in common with those up regulated from D to IC.

```{r}
Common_markers$`R-D vs R-IC vs IC-D UP`$intersections$`R-D ∩ IC-D`
```

-   We have 106 genes in common with those up regulated from IC to R (here's the top50).

```{r}
Common_markers$`R-D vs R-IC vs IC-D UP`$intersections$`R-D ∩ R-IC` %>% head(50)
```

-   We have 3 common genes between all the stages

```{r}
Common_markers$`R-D vs R-IC vs IC-D UP`$intersections$`R-D ∩ R-IC ∩ IC-D`
```
...
```{r}
Common_markers$`R-D vs R-IC vs IC-D Down`$plot
```

__We have 718 down regulated genes from D to R. __

-   We have 78 genes in common with those down regulated from D to IC (here's the top50).

```{r}
Common_markers$`R-D vs R-IC vs IC-D Down`$intersections$`R-D ∩ IC-D` %>% head(50)
```

-   We have 118 genes in common with those down regulated from IC to R (here's the top50).

```{r}
Common_markers$`R-D vs R-IC vs IC-D Down`$intersections$`R-D ∩ R-IC` %>% head(50)
```

-   We have 2 common genes between all the stages

```{r}
Common_markers$`R-D vs R-IC vs IC-D Down`$intersections$`R-D ∩ R-IC ∩ IC-D`
```

<p class='p2'>his means that from diagnosis (D) to the intermediate cluster (IC), we observe 78 genes that are down-regulated. From IC to relapse (R), 2 of these previously down-regulated genes maintained their down-regulation, alongside 116 additional genes. Furthermore, 524 genes were uniquely down-regulated in the R-D analysis, suggesting that this regulation likely occurred after the blood was sampled for the analysis.</p>
...
```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 UP`$plot
```

__We have 50 up regulated genes from D to R. __

-   We have 11 genes in common with those up regulated from D to IC.

```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 UP`$intersections$`Rcl1-Dcl1 ∩ ICcl1-Dcl1`
```

-   We have 32 genes in common with those up regulated from IC to R.

```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 UP`$intersections$`Rcl1-Dcl1 ∩ Rcl1-ICcl1` %>% head(50)
```

-   We have 1 common gene between all the stages

```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 UP`$intersections$`Rcl1-Dcl1 ∩ Rcl1-ICcl1 ∩ ICcl1-Dcl1`
```

<p class='p2'>his means that from diagnosis (D) to the intermediate cluster (IC), we observe 11 genes that are up-regulated. From IC to relapse (R), 1 of these previously up-regulated genes maintained its up-regulation, alongside 31 additional genes. Furthermore, 8 genes were uniquely up-regulated in the R-D analysis, suggesting that this regulation likely occurred after the blood was sampled for the analysis.</p>
...
```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 Down`$plot
```

__We have 439 down regulated genes from D to R. __

-   We have 39 genes in common with those down regulated from D to IC.

```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 Down`$intersections$`Rcl1-Dcl1 ∩ ICcl1-Dcl1` %>% head(50)
```

-   We have 61 genes in common with those down regulated from IC to R (here's the top50).

```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 Down`$intersections$`Rcl1-Dcl1 ∩ Rcl1-ICcl1` %>% head(50)
```

-   We have 1 common gene between all the stages

```{r}
Common_markers$`Rcl1-Dcl1 vs Rcl1-ICcl1 vs ICcl1-Dcl1 Down`$intersections$`Rcl1-Dcl1 ∩ Rcl1-ICcl1 ∩ ICcl1-Dcl1`
```

<p class='p2'>his means that from diagnosis (D) to the intermediate cluster (IC), we observe 39 genes that are down-regulated. From IC to relapse (R), 1 of these previously down-regulated genes maintained their down-regulation, alongside 60 additional genes. Furthermore, 340 genes were uniquely down-regulated in the R-D analysis, suggesting that this regulation likely occurred after the blood was sampled for the analysis.</p>



### 2-2- Inspecte common genes

<p class='important'>Now, let's inspect the common enriched pathways between all the groups</p>
